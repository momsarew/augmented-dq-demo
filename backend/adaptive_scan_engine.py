...
- Catalogue étendu : 60 anomalies (15 par dimension)
- Apprentissage adaptatif automatique
- Priorisation intelligente
- Export résultats pour calcul vecteurs 4D
"""
...
from extended_anomaly_catalog import ExtendedCatalogManager, CoreAnomaly
...
class ScanReport:
    ...
    def get_beta_parameters(self) -> Dict:
        """
        Calcule paramètres Beta (α, β) par dimension depuis résultats scan
        
        Returns:
            Dict avec structure:
            {
                'DB': {'alpha': X, 'beta': Y, 'total_rows': N},
                'DP': {'alpha': X, 'beta': Y, 'total_rows': N},
                'BR': {'alpha': X, 'beta': Y, 'total_rows': N},
                'UP': {'alpha': X, 'beta': Y, 'total_rows': N}
            }
        
        Logique:
            - alpha = Nombre lignes avec anomalies détectées (échecs)
            - beta = Nombre lignes sans anomalies (succès)
            - Si 0 anomalie détectée → alpha=0, beta=N → Quasi-perfection
        """
        by_dim = {}
        
        # Initialiser toutes dimensions à 0
        for dim in ['DB', 'DP', 'BR', 'UP']:
            by_dim[dim] = {
                'alpha': 0,  # Lignes avec anomalies
                'beta': self.total_rows,  # Lignes clean (par défaut tout)
                'total_rows': self.total_rows
            }
        
        # Compter anomalies par dimension
        for result in self.results:
            if result.detected and result.affected_rows > 0:
                dim = result.anomaly_id.split('#')[0]
                if dim in by_dim:
                    by_dim[dim]['alpha'] += result.affected_rows
        
        # Ajuster beta (lignes clean = total - anomalies)
        for dim in by_dim:
            by_dim[dim]['beta'] = self.total_rows - by_dim[dim]['alpha']
            # Éviter beta=0 (au moins 1)
            if by_dim[dim]['beta'] < 1:
                by_dim[dim]['beta'] = 1
    ...
    def _auto_detect_params(self, df: pd.DataFrame, anomaly: CoreAnomaly) -> Dict:
        ...
        # BR#2: Hors bornes métier
            ...
            if 'age' in df.columns:
                params['column'] = 'age'
                # Utiliser quantiles au lieu de valeurs hardcodées
                params['min_val'] = float(df['age'].quantile(0.01))
                params['max_val'] = float(df['age'].quantile(0.99))
            elif 'salaire' in df.columns:
                params['column'] = 'salaire'
                # Utiliser quantiles au lieu de valeurs hardcodées
                params['min_val'] = float(df['salaire'].quantile(0.01))
                params['max_val'] = float(df['salaire'].quantile(0.99))
            else:
                numeric_cols = df.select_dtypes(include=['number']).columns.tolist()
                if numeric_cols:
                    col = numeric_cols[0]
