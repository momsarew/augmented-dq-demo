...
class DAMACalculator:
    ...
    def compute_dama_score(self, 
        ...
        # ============================================================================
        # Pour DAMA, l'unicité mesure l'absence de doublons EXACTS dans la colonne
        # Formule : 1 - (nb_lignes_dupliquées / total)
        #
        # Exemple : [A, B, A, C, C, C] (6 lignes)
        #   - A apparaît 2x → 1 ligne en trop (doublon)
        #   - C apparaît 3x → 2 lignes en trop (doublons)
        #   - Total doublons = 3 lignes
        #   - Uniqueness = 1 - 3/6 = 50%
        #
        # Si aucun doublon → 100%
        if total > 0:
            # Compter les lignes qui sont des doublons (apparaissent plus d'une fois)
            duplicated_count = series.duplicated(keep='first').sum()
            uniqueness = 1 - (duplicated_count / total)
        else:
            uniqueness = 0.0
