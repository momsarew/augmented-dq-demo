...
def simulate_lineage(vector_4d_source: Dict[str, float],
    ...
    """
    # Pipeline par défaut: étapes sans risque additionnel
    # NOTE: Les vrais risques de pipeline doivent être configurés par l'utilisateur
    # Ce pipeline par défaut montre la PROPAGATION sans AJOUT de risque simulé
    if pipeline_config is None:
        pipeline_config = [
            {
                "nom": "ETL Extraction",
                "description": "Extraction données source",
                "P_DB_add": 0.0,  # Pas de risque simulé
                "P_DP_add": 0.0,  # Pas de risque simulé
                "P_BR_add": 0.0,
                "P_UP_add": 0.0
            },
            {
                "nom": "Transformation",
                "description": "Transformations métier",
                "P_DB_add": 0.0,
                "P_DP_add": 0.0,
                "P_BR_add": 0.0,
                "P_UP_add": 0.0
            ...
            {
                "nom": "Chargement",
                "description": "Chargement destination",
                "P_DB_add": 0.0,
                "P_DP_add": 0.0,
                "P_BR_add": 0.0,
                "P_UP_add": 0.0
        ...
        # NOTE: Pour voir des dégradations réelles, l'utilisateur doit
        # configurer les P_*_add selon ses pipelines ETL réels
