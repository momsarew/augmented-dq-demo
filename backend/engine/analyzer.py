...
def detect_business_violations(series: pd.Series, col_name: str) -> Dict[str, Any]:
    ...
    if 'anciennete' in col_name.lower():
        # Détecter grandes divergences (placeholder - nécessiterait Date_anciennete)
        # Pour l'instant, estimation basique
        violations["violation_count"] = int(len(series) * 0.05)  # 5% estimé
        violations["rules_violated"].append("incoherences_calcul")
...
def compute_column_quality_metrics(series: pd.Series) -> Dict[str, float]:
    ...
    # Validité (approximation - inverse du taux d'erreur)
    validity = 0.95  # Placeholder - sera affiné par detect_type_errors
