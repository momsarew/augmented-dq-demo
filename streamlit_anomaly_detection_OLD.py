...
from core_anomaly_catalog import CoreCatalogManager, CoreAnomaly, Dimension, Criticality
...
def render_anomaly_detection_tab():
    ...
    with sub_tab1:
        ...
        ⚡ **3 budgets** : QUICK (top 5) | STANDARD (top 10) | DEEP (les 15)
        """)
        
        # Upload fichier
        uploaded_file = st.file_uploader(
            "📂 Charger dataset (CSV, Excel)",
            type=['csv', 'xlsx', 'xls'],
            help="Dataset à analyser pour détecter anomalies"
        )
            ...
            try:
                ...
                with col1:
                    budget = st.selectbox(
                        ...
                        index=1,
                        help="QUICK=Top 5 | STANDARD=Top 10 | DEEP=Les 15"
                    )
    ...
    with sub_tab3:
        ...
        if not stats_df.empty:
            ...
            with col3:
                avg_freq = stats_df['Fréquence'].str.rstrip('%').astype(float).mean() / 100
                st.metric("Fréquence moyenne", f"{avg_freq:.1%}")
            
            with col4:
                avg_score = stats_df['Score Priorité'].astype(float).mean()
            ...
            with col1:
                # Top 10 fréquences
                top_freq = stats_df.nlargest(10, 'Score Priorité')
                fig = px.bar(
                    top_freq,
                    x='ID',
                    y='Score Priorité',
            ...
            with col2:
                ...
                freq_by_dim = stats_df.groupby('Dimension')['Fréquence'].apply(
